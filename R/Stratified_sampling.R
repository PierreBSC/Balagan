...
#' @param name_stratifying_marker Name of the gene used for the stratification. Not essential, only for visualisation purpose
...
Stratified_sampling = function(Thresholded_image,sce,N_FoV_per_region,FoV_size = 100,N_sampling = 50,
                               Weight_strata=NULL,List_strata = NULL,Parallel_computing=FALSE,name_stratifying_marker = NULL) {
  ...
  if (Parallel_computing) {
    ...
    Stratified_sampling_estimation =foreach(i=colnames(Transformed_data),.combine = rbind) %dopar% {
      ...
      Stratified_sampling_estimation_temp
    }
  ...
  boxplot(Stratified_sampling_estimation,outline=F,xlab="Cell phenotype",ylab="Estimated cell density",
          main=paste("Results of stratified sampling using",name_stratifying_marker))
